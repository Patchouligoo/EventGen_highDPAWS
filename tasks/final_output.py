...
from tasks.signal_generation import OmniLearnSignalPrep, NEventsMixin, DecayChannelMixin, BaseTask
    ...
    def run(self):
            ...
            for my in self.my_values:
                ...
                if df_SR_data_sig_constituents is None:
                    with h5.File(SR_data_sig_path_mxmy, "r") as hf:
                        ...
                        df_SR_data_sig_param_mx = np.full(hf["constituents"][:].shape[0], mx)
                        df_SR_data_sig_param_my = np.full(hf["constituents"][:].shape[0], my)
                    ...
                    with h5.File(SR_data_sig_path_mxmy, "r") as hf:
                        ...
                        df_SR_data_sig_param_mx = np.concatenate(
                            (df_SR_data_sig_param_mx, np.full(hf["constituents"][:].shape[0], mx)), axis=0
                        )
                        df_SR_data_sig_param_my = np.concatenate(
                            (df_SR_data_sig_param_my, np.full(hf["constituents"][:].shape[0], my)), axis=0
                        )

        print("Total number of available signal events for data: ", df_SR_data_sig_constituents.shape[0])
            ...
            for my in self.my_values:
                ...
                if df_SR_MC_sig_constituents is None:
                    with h5.File(SR_MC_sig_path_mxmy, "r") as hf:
                        ...
                        df_SR_MC_sig_param_mx = np.full(hf["constituents"][:].shape[0], mx)
                        df_SR_MC_sig_param_my = np.full(hf["constituents"][:].shape[0], my)
                    ...
                    with h5.File(SR_MC_sig_path_mxmy, "r") as hf:
                        ...
                        df_SR_MC_sig_param_mx = np.concatenate(
                            (df_SR_MC_sig_param_mx, np.full(hf["constituents"][:].shape[0], mx)), axis=0
                        )
                        df_SR_MC_sig_param_my = np.concatenate(
                            (df_SR_MC_sig_param_my, np.full(hf["constituents"][:].shape[0], my)), axis=0
                        )

        print("Total number of available signal events for MC: ", df_SR_MC_sig_constituents.shape[0])
        ...
        with h5.File(SR_data_bkg_path, "r") as hf_in:
            with h5.File(self.output()["SR_data_bkg"].path, "w") as hf_out:
                ...
                print("Total number of available background events for data in SR: ", hf_in["constituents"][:].shape[0])
        ...
        with h5.File(SB_data_bkg_path, "r") as hf_in:
            with h5.File(self.output()["SB_data_bkg"].path, "w") as hf_out:
                for key in hf_in.keys():
                    hf_out.create_dataset(key, data=hf_in[key][:])
                print("Total number of available background events for data in SB: ", hf_in["constituents"][:].shape[0])
        ...
        with h5.File(SR_MC_bkg_path, "r") as hf_in:
            with h5.File(self.output()["SR_MC_bkg"].path, "w") as hf_out:
                for key in hf_in.keys():
                    hf_out.create_dataset(key, data=hf_in[key][:])
                print("Total number of available background events for MC in SR: ", hf_in["constituents"][:].shape[0])
